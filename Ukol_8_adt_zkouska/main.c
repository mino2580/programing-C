...
/////////////////////////////////////////////////////////////////////////////
void Merge2(List_t *vys, List_t *s2, List_t *s1)
{
  //  serad(s1);
   // serad(s2);
    printf("\nseznam 1...\n");
    ...
    List_First(s2);//
	 int cmp=0;
    while(s1->active!=NULL )
	{
		
		while(s2->active!=NULL )
	    {
			cmp=Data_cmp(&s1->active->data, &s2->active->data);
            if(cmp <0){ //s1 je mensi
                 List_Post_Insert(vys, s1->active->data);
                List_Succ(s2);
...
///////////////////////////////////////////////////////////////////////////////////
void Merge(List_t *vys, List_t *s2, List_t *s1){
  //  serad(s1);
   // serad(s2);
    printf("\nseznam 1...\n");
	print_seznam(s1);
    ...
    while(s1->active!=NULL)
	{
		...
		else
		{
            ...
            if(cmp <0){ //s1 je mensi
                ...
                List_Post_Insert(vys, s1->active->data);
                List_Succ(s2);//Posune aktivitu na následující prvek seznamu.
            }
        }
...
/*********************************************************************/			
    ...
    printf("Ukazka funkcnosti - ListMerge\n");
   ...
   // printf("\nseznam 2...\n");
    List_Insert_First(&s1, vloz_data("a1", 4, 68.4, 180));
    List_Insert_First(&s1, vloz_data("a2", 3, 82.7, 178));
    List_Insert_First(&s1, vloz_data("a3", 2, 77.4, 186));
    List_Insert_First(&s1, vloz_data("a4",1, 61.8, 168));
    //print_seznam(&s1);

   // printf("\nseznam 1...\n");
    List_Insert_First(&s2, vloz_data("b1", 8, 58.4, 167));
    List_Insert_First(&s2, vloz_data("b2", 7, 60.2, 173));
    List_Insert_First(&s2, vloz_data("b3", 6, 58.7, 177));
    List_Insert_First(&s2, vloz_data("b4", 5, 48.9, 167));
   // print_seznam(&s2);
