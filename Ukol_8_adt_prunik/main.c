...
void prunik2( List_t *s1, List_t *s2)
{
    // serad(s1);
   // serad(s2);
	//print_seznam(s1);
    ...
    while(s1->active!=NULL )
    {
		  ...
		  while(s2->active!=NULL ) 
		{
			cmp=Data_cmp(&s1->active->data, &s2->active->data);
			 if (cmp==0)  List_Succ(s2);
		  
		    else if  (cmp==1)
            {
			      if (s1->first != s1->active)  
				    {
					   while(s1->first != s1->active) {//maye sa aj uloyenz
					   List_Delete_First(s1); }
					   
					   s1->active->next=s1->active;
					    
			        }
					
			       else if (s1->first == s1->active)  List_Succ(s2);
